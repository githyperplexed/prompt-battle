...
runs a contest: create → ingest → dq → score → cluster → advance → publish, plus reset /
delete / status / smoke), and `web` (SvelteKit site that renders contest state read-only).
...
| [secrets/](secrets/)               | **Gitignored keyword secrets**, one `secrets/<videoId>.json` per contest (format in `example.json`): the hidden keywords + salt.                                                   | Required locally by `create`, `ingest`, `cluster`, and `publish`. Only the salted hash is ever committed or stored; never commit the plaintext.                                                                                                                                                                                                                |
